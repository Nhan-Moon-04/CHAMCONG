...
from .services.salary_importer import import_salary_detail_file, import_salary_file
...
def register_routes(app):
    ...
    def import_salaries():
        ...
        try:
            ...
            detail_result = import_salary_detail_file(
                str(temp_path),
                upload.filename,
                actor=actor,
                target_month=month_key,
            ...
            if detail_result.get("detected"):
                leave_stats = detail_result.get("leave", {})
                slip_stats = detail_result.get("slips", {})
                ins_stats = detail_result.get("insurance", {})
                flash(
                    "Đã tách dữ liệu chi tiết: "
                    f"phép năm (+{leave_stats.get('created', 0)}/{leave_stats.get('updated', 0)}), "
                    f"phiếu lương (+{slip_stats.get('created', 0)}/{slip_stats.get('updated', 0)}), "
                    f"BHXH (+{ins_stats.get('created', 0)}/{ins_stats.get('updated', 0)}).",
                    "success",
                )
            return redirect(url_for("salaries", month=month_key))
